...
```{r}
weights_normalized <- lapply(constrained_weights, function(df){
...
weighted_ensemble <- Map(merge, constrained_result, weights_normalized, by = "run_number")

# saving as a list does this work?
saveRDS(weighted_ensemble, "data/weighted_constrained_ensemble.RDS")
...
eoc_warming_results <- lapply(names(gsat_95_to_14), function(df_name){
  
  # extract data by name 
  df <- gsat_95_to_14[[df_name]]
...
eoc_gsat_ensemble <- Map(merge, eoc_warming_results, weights_normalized, by = "run_number")

# save the result for future visualization
saveRDS(eoc_gsat_ensemble, "data/weighted_gsat_metrics.RDS")
...
names(temp_probability) <- names(eoc_gsat_ensemble)
...
The last thing that we will produce with thhis set of code is a probability result where the probability is computed for a sequence of temperature ranges. To complete this we will use temperature bins defined as a sequence. 

Here I define the temperature sequence and save the result by name, but will use the previous code chunk to run the analysis.

```{r}
temp_seq <- c(seq(0, 10, by = 0.1))

saveRDS(temp_probability_sequence, "data/full_sequence_temp_probability.rds")
```

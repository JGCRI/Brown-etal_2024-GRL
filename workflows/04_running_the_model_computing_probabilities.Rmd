...
names(eoc_global_tas_ensemble) <- names(weighted_ensemble)

# define bins that represent the warming ranges of interest
temp_ranges <- c(0, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, Inf)

# computing probabilities 
temp_probability <- lapply(names(eoc_global_tas_ensemble), function(df_name){
  
  # copy data based on element name
  df <- eoc_global_tas_ensemble[[df_name]]
...
names(temp_probability) <- names(eoc_global_tas_ensemble)

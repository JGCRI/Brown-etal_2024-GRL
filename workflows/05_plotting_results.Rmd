...
eoc_gsat <- lapply(names(gsat_results), function(df_name) {
  
  data <- gsat_results[[df_name]]
...
warming_projection <- eoc_gsat_df %>% 
  ...
  summarize(
    ...
    ci_16 = weighted.quantile(x = value, w = mc_weight, probs = 0.16), 
    ci_84 = weighted.quantile(x = value, w = mc_weight, probs = 0.84), 

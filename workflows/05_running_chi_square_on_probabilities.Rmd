...
resampled_metrics <- lapply(warming_metrics, function(df) {
  
  #new samples 
  new_sample <- sample(df$metric_result, size = 10000, replace = T, prob = df$norm_weght)
...
pairs <- combn(seq_along(resampled_metrics), 2, simplify = FALSE)
...
compute_ks_test <- function(pair) {
  ...
  dist1 <- metric_results[[pair[1]]]
  dist2 <- metric_results[[pair[2]]]
...
```{r}
Baseline <- data.frame(metric = resampled_metrics$Baseline, scenario = "Baseline")
IPCC <- data.frame(metric = resampled_metrics$IPCC, scenario = "IPCC")
No_Process <- data.frame(metric = resampled_metrics$No_Process, scenario = "No_Process")
No_Historical <- data.frame(metric = resampled_metrics$No_Historical, scenario = "No_Historical")
No_Paleoclimate <- data.frame(metric = resampled_metrics$No_Paleoclimate, scenario = "No_Paleoclimate")
baseline_EC <- data.frame(metric = resampled_metrics$Baseline_Emergent_constraints, scenario = "Baseline_Emergent_constraints")
...
CDF_temp <- ggplot(cdf_df, 
  ...
  labs(title = "CDFs of Two Distributions", x = "Value", y = "CDF") +
  theme_light()
